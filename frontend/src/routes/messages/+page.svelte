...
<div class="mx-auto max-w-3xl px-4 py-8">
	<h1 class="font-display text-primary mb-8 text-center text-4xl">Messages</h1>
	...
	>
		...
		<span class="font-display text-lg">View the Gallery</span>
	...
	<!-- Message form -->
	<div class="border-accent/30 mb-10 rounded-lg border bg-white/80 p-6 shadow-md">
		<h2 class="font-display text-primary mb-4 text-2xl">Leave a Message</h2>
		<form onsubmit={handleSubmit} class="space-y-4">
			<div>
				<label for="author" class="text-primary mb-1 block text-sm font-medium">Your name</label>
				<input
					id="author"
					type="text"
					bind:value={author}
				...
				{#if photoPreview}
					<div class="flex items-center gap-3">
						<img
							src={photoPreview}
							alt="Preview"
							class="border-accent/40 h-16 w-16 rounded-full border-2 object-cover"
						/>
						<button
							type="button"
							onclick={removePhoto}
			...
			{#if submitted}
				<p class="text-accent text-sm">Thank you for your message!</p>
			{/if}
			{#if error}
	...
	{:else}
		<div class="space-y-4">
			{#each messages as msg (msg.id)}
				<div class="border-accent/20 rounded-lg border bg-white/70 p-5 shadow-sm">
					<p class="text-primary text-lg leading-relaxed">{msg.text}</p>
						...
						{#if msg.photoUrl}
							<button onclick={() => (lightboxUrl = msg.photoUrl || '')}>
								<img
									...
									class="border-accent/30 hover:border-accent h-10 w-10 rounded-full border-2 object-cover transition-colors"
								/>
							</button>
						{/if}
